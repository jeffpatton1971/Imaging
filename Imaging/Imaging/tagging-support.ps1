...
<#
    ...
    #>
param([string]$d, [string]$f) 
    [DateTime]::ParseExact($D,$f,[System.Globalization.CultureInfo]::InvariantCulture) 
}
...
Param  ([Parameter(Mandatory=$true )][Alias("Filename","FullName")]$Path , $offset) 
        ...
        Import-Csv -Path $path -Header "Lon","Lat","blank","Date", "Knots","bearing","altitude" |
            select  -property  @{Name="DateTime"; Expression = {(   ConvertTo-DateTime $_.Date $DateFormat ).addSeconds($offset) }}  ,
                               @{Name="MPH";      Expression = {    [system.math]::Round((1.15  * $_.Knots),1) }} , @{Name="KPH"; Expression = { [system.math]::Round((1.852 * $_.Knots),1) }} ,
                               knots, bearing ,          

...
function Copy-Exif{
    ...
    process {
        $destinationPath | foreach {               & 'C:\Program Files (x86)\EXIFutils\exifcopy.exe' "/bq" "/o" $SourcePath $_
                                    ...
                                    if ($FixICE) { & 'C:\Program Files (x86)\EXIFutils\exifedit'    "/b"   "/a" "Firm-ver=Microsoft ICE v1.3.5.0" "/r" "orient" "/s" "/t" "a,160" $DestinationPath }
                                    }
    }
}
...
function Fix-ICeExif{
    Param  ( [Parameter(Mandatory=$true  ,valueFromPipeline=$true )] $DestinationPath)
& 'C:\Program Files (x86)\EXIFutils\exifedit'     "/b"   "/a" "Firm-ver=Microsoft ICE v1.3.3.0" "/r" "orient" "/s"  "/t" "a,160" $DestinationPath 
}
